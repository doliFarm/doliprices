...
include "key.php";  // required to generate the key for the file
...
<html>
		<head>
			<title>Prices List generator - Mandala, oct 2021 - ver 1.0-1</title>
				<!-- Required meta tags -->
...
echo '
		<div class="card">
				<div class="card-img-top d-flex align-items-center bg-light">
						...
						<h5 class="card-title">Génération automatique d\'offres clients (v1.0)</h5>
		...
		<p class="col p-2 m-0">
		   <h4 class="text-end">Numéro de semaine: *'. $week .'* ('. $date->setISODate(date("Y"), $week, "1")->format('d/m/Y')." - ".$date->setISODate(date("Y"), $week, "5")->format('d/m/Y').')</h4>
		...
		<p>
			...
			<form action="index.php" method="post" id="loadFileForm" enctype="multipart/form-data">
				2) Chargez le fichier enregistré sur le bureau: <input type="file" name="fileToUpload" id="fileToUpload"> 
				<button id="loadFilebtn" name="submit" class="btn btn-primary" type="submit" value="submit">Carica file</button>
			</form>
		</p>';
...
<?php
	    ...
	    echo '<p class="text-end">Origine de la liste des prix: '.$messaggioOrigineDati.'<p>';
		...
		echo '<ul class="nav nav-tabs" id="myTab" role="tablist">
				  <li class="nav-item" role="presentation">
					<button class="nav-link active" id="listaPrezzi-tab" data-bs-toggle="tab" data-bs-target="#listaPrezzi" type="button" role="tab" aria-controls="listaPrezzi" aria-selected="false">Liste des prix actuels</button>
				  </li>
				  <li class="nav-item" role="presentation">
					<button class="nav-link" id="news-tab" data-bs-toggle="tab" data-bs-target="#news" type="button" role="tab" aria-controls="news" aria-selected="true">Nouvelles des producteurs</button>
				  </li>
				  <li class="nav-item" role="presentation">
					<button class="nav-link " id="invioOfferte-tab" data-bs-toggle="tab" data-bs-target="#invioOfferte" type="button" role="tab" aria-controls="invioOfferte" aria-selected="false">Envoi d\'offres</button>
				  </li>
				  <li class="nav-item" role="presentation">
...
echo '     <hr>';
echo '	   <h4>Nouvelles des producteurs</h4> ';

require_once '../mandalanews/letturanews.php';
letturaNews($week);
...
foreach( $clienti as $cliente )
{
    ...
    if (($cliente[0]!=NULL) && (strtoupper($cliente[0])!='CLIENTS')) {  // Avoid empty rows && header rows
			...
			echo '<tr>';
				...
				$outputOfferClient[strtoupper($cliente[0])] = '../offers/'.$week.'_ListinoPrezziMandala_'.$cliente[0].'_'.$chiavi[strtoupper($cliente[0])].'.xlsx';
				$outputOfferClientMarge[strtoupper($cliente[0])] = '../offers/'.$week.'_ListinoPrezziMandala_'.$cliente[0].'_'.$chiavi[strtoupper($cliente[0])].'_marge.xlsx';
				...
				if (file_exists($outputOfferClientMargeConfirmed[strtoupper($cliente[0])])) {
					$linkConfirmedMarge[strtoupper($cliente[0])] = '../ordersConfirmedMarge/'.$week.'_ListinoPrezziMandala_'.$cliente[0].'_'.$chiavi[strtoupper($cliente[0])].'_marge.xlsx';
        		} else {
					$linkConfirmedMarge[strtoupper($cliente[0])] = '../offers/'.$week.'_ListinoPrezziMandala_'.$cliente[0].'_'.$chiavi[strtoupper($cliente[0])].'_marge.xlsx';
				}
				...
				*/
				...
				echo '<td><p> 
                        ...
                        if (strtoupper($cliente[1]) == "JUSTIN" ) {
							$messaggioCliente = "Dear $cliente[1], <br> Please, find enclosed the price list for this week. <br><br>
													...
													1755 Gooik<br>
													Belgium							";
							$subjectEmail = "Mandala - PRICE LIST WEEK $week ";

						} else if (strtoupper($cliente[1]) == "ROBBY" ) { 
						        $messaggioCliente = "Bonjour $cliente[1], <br> Please, find the price list for this week:  http://www.mandalaorganicgrowers.com/lettura_file.php?k=".$chiavi[strtoupper($cliente[0])].". <br><br>
														<br><br>
														Anne Marie <br>
														 <br>
														Mobile :  0032 495 61 97 51 <br>
				...
				echo '<p style="background-color:#f8f8ff"> 
							 <form id="formMail_'.$cliente[0].'">
								 ...
								 <input type="hidden" id="formXLSfile_'.$cliente[0].'" name="formXLSfile_'.$cliente[0].'" value="'.$outputOfferClient[strtoupper($cliente[0])].'">
									...
									<div class="mb-3">
									  <label for="exampleFormControlInput1" class="form-label">Email address (addresses separated by commas)</label>
									  <input type="email" class="form-control" id="formEmail_'.$cliente[0].'" value="'.$cliente[2].'">
									</div>
									<div class="mb-3">
									  <label for="exampleFormControlInput1" class="form-label">Email CC (addresses separated by commas)</label>
									  <input type="email" class="form-control" id="formCC_'.$cliente[0].'" value="mandala@mandalaorganicgrowers.com">
									</div>
									<div class="mb-3">
...
echo '<h3>liste des prix actuels</h3> ';
